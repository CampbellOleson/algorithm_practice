...
function sumArray(array) {
  ...
  return array[0] + sumArray(array.slice(1, array.length)); // note: Array.slice last num is non-inclusive
}
...
function fileFinder(directories, targetFile) {
  for (let dir in directories) {
    console.log(dir);
    // the 'in' operator checks to see if a property is in an objects prototype chain
    if (
      dir === targetFile || // if the directory is the file
      fileFinder(directories[dir], targetFile) === true // or the recirsive call returned true
    ) {
      return true;
    }
  }
...
// fileFinder(desktop, "express_yourself.wav");
