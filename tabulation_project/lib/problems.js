...
function stepper(nums, memo = {}) {
  const key = String(nums.length); // the length varies with every nums array
  if (key in memo) return memo[key];
  if (nums.length < 1) return true;
  let maxSteps = nums[0];
  for (let step = 1; step <= maxSteps; step++) {
    const result = stepper(nums.slice(step), memo);
    memo[key] = result;
    if (result) {
      return true;
    }
...
function maxNonAdjacentSum(nums) {
  ...
  for (let i = 0; i <= tab.length; i++) {
    const skipLeft = tab[i - 2] ? tab[i - 2] : 0;
    const include = skipLeft + nums[i];
    const notInclude = tab[i - 1];
    Math.max(include, notInclude);
  }
  return tab[tab.length - 1];
}
